...
def FirstLayerDMM(prompt: str="test"):
    ...
    def api_call_with_timeout():
        nonlocal response, timeout_occurred
        try:
    ...
    if api_thread.is_alive():
        print("⚠️ Cohere API call timed out, using fallback")
        timeout_occurred = True
    
    if timeout_occurred or not response:
        # Fallback to simple keyword-based classification
        prompt_lower = prompt.lower()
        if any(word in prompt_lower for word in ["open", "launch", "start"]):
            return ["open " + prompt.split()[-1]]
        elif any(word in prompt_lower for word in ["close", "stop", "exit"]):
            return ["close " + prompt.split()[-1]]
        elif any(word in prompt_lower for word in ["play", "music", "song"]):
            return ["play " + " ".join(prompt.split()[1:])]
        elif any(word in prompt_lower for word in ["search", "find", "look up"]):
            return ["google search " + " ".join(prompt.split()[1:])]
        else:
...
def ChatBot(prompt: str):
    """ChatBot function for general AI responses, including email generation"""
    try:
        # Use the same Cohere client for general chat
        stream = co.chat_stream(
            model="command-r-plus",
            message=prompt,
            temperature=0.7,
        ...
        print(f"⚠️ ChatBot API error: {e}")
        # Return a simple fallback response
        return f"I apologize, but I'm unable to generate a response at the moment. Please try again later."
