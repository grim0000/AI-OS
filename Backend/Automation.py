...
def open_desktop_app(app_path):
    ...
    try:
        # Method 1: Try direct subprocess
        subprocess.Popen([app_path], shell=True)
        return True
    except Exception as e:
        print(f"Direct subprocess failed: {e}")
        
        try:
            # Method 2: Try using os.startfile for Windows
            os.startfile(app_path)
            return True
        except Exception as e2:
            print(f"os.startfile failed: {e2}")
            
            try:
                # Method 3: Try using shell=True with quotes
                subprocess.Popen(f'"{app_path}"', shell=True)
                return True
            except Exception as e3:
                print(f"Quoted subprocess failed: {e3}")
                
                try:
                    # Method 4: Try using start command for Windows Store apps
                    if "WindowsApps" in app_path:
                        # Extract app name from Windows Store path
                        app_name = app_path.split("\\")[-1].replace(".exe", "")
                        subprocess.run(["start", app_name], shell=True)
                        return True
                except Exception as e4:
                    print(f"Start command failed: {e4}")
                    
                    try:
                        # Method 5: Try using explorer to open
                        subprocess.run(["explorer", app_path], shell=True)
                        return True
                    except Exception as e5:
                        print(f"Explorer method failed: {e5}")
                        return False
...
def find_desktop_app(app_name):
    """Find desktop application executable with enhanced Windows Store support"""
                ...
                if open_spotify_specific():
                    ...
                    try:
                        ...
                        if spotify_desktop_search(music_query, skip_search=True):
                            return f"Searched and attempted to play '{music_query}' in Spotify desktop app"
                        else:
                            return f"Searched for '{music_query}' in Spotify desktop app"
                        ...
                        if spotify_desktop_search(music_query):
                            ...
                            return f"Opened Spotify desktop app. Please search manually for: {music_query}"
                else:
                    ...
                    webbrowser.open(search_url)
                    return f"Opened Spotify web and searching for: {music_query}"
            else:
...
def OpenApp(app_command):
    """Enhanced app opening with desktop app priority and Windows Store support"""
    try:
        ...
        if app_name == "spotify":
            print("Using specialized Spotify opening method")
            if open_spotify_specific():
                return f"Opened Spotify desktop application"
            else:
                # Fallback to web version
                if open_web_app(app_name, query):
                    return f"Opened Spotify web application"
        ...
        try:
            ...
            search_url = f"https://www.google.com/search?q={app_name}"
            webbrowser.open(search_url)
            return f"Could not find {app_name}, opened Google search instead"
...
def smart_query_classifier(query: str) -> dict:
    ...
    if any(word in query_lower for word in ["play", "music", "song", "track", "album", "listen"]):
...
# Enhanced Query Classifier with Spotify Controls
def enhanced_query_classifier(query: str) -> dict:
    """
    Enhanced query classifier that includes Spotify controls
    """
    query_lower = query.lower()
    result = {
        "action": None,
...
def execute_enhanced_command(classification: dict) -> str:
    """
    Execute commands with enhanced Spotify controls
    """
    try:
        action = classification["action"]
        target = classification["target"]
